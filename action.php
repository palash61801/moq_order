			...
			echo "
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-heading'>$.$pro_price.00  MOQ: $pro_moq
...
if(isset($_POST["get_seleted_Category"]) || isset($_POST["selectBrand"]) || isset($_POST["search"])){
	...
	if(isset($_POST["addToCart"])){
		...
		$product_query = "SELECT product_moq FROM products where product_id = '$p_id' LIMIT 1";
	    $run_query = mysqli_query($con,$product_query);
	    if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
		     $pro_moq = $row['product_moq'];}
		...
		if($count > 0){
			...
			(`p_id`, `ip_add`, `user_id`, `qty`, `p_moq`) 
			VALUES ('$p_id','$ip_add','$user_id','',$pro_moq)";
				...
				if (!isset($_SESSION["uid"])) {
					...
					echo '
						...
						<form action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post">
							...
							<input type="hidden" name="business" value="miabhaistore1189800@business.example.com">
							...
							echo   
								'<input type="hidden" name="return" value="http://localhost/moqorder/payment_success.php"/>
					                <input type="hidden" name="notify_url" value="http://localhost/moqorder/payment_success.php">
									<input type="hidden" name="cancel_return" value="http://localhost/moqorder/cancel.php"/>
...
if (isset($_POST["updateCartItem"])) {
	...
	$sql = "SELECT * FROM cart WHERE p_moq > '$qty' AND p_id = '$update_id' LIMIT 1" ;
	...
	if(mysqli_num_rows($check_query) > 0){
		...
		echo "
			<div class='alert alert-danger'>
				<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
				<b>Minimum $pro_moq quantity required</b>
			</div>
		";
		exit();
	}
	...
	if (isset($_SESSION["uid"])) {
		...
		echo "<div class='alert alert-info'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Product is Added and Ready to Chackout</b>
				</div>";
		exit();
